package de.tum.moodtrip_backend.adapter.content.chatbot.service;
...
@Service
public class EmotionService implements EmotionPort {

    private static final String EMOTION_DETECTION_PROMPT = """
            You are an emotion classifier. You receive as input the *entire conversation history* between USER and BOT plus the latest user message. The text will look like:
            ...
            Using all user messages in the history (not only the last one), infer the user's current overall emotional state and produce a compact single-line JSON with:
    ...
    public EmotionService(DeepSeekChatModel chatModel, ConversationPort conversationPort) {
